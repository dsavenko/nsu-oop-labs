...
std::cout << t; 
```
Вы ограничены возможностями стандарта С++11. Это означает, что нельзя, например, использовать `if constexpr`, `std::integer_sequence/std::index_sequence` и fold expressions, т.к. они появились в более поздних версиях. _Этот запрет действует только здесь. В допах этой подзадачи, а также в следующих подзадачах его нет._
