...
> [!NOTE]
> 1. Если в первой задаче вы не сделали итераторы, реализуйте их сейчас. Итераторы - часть минимальных требований данной задачи.
> 2. Если вы не делали первую задачу совсем, вам нужно сразу написать шаблонный класс FlatMap, удовлетворяющий минимальным условиям 1-й задачи + итераторы. Другие допы первой задачи делать не обязательно.
> 3. В первой задаче был запрет на использование стандартных контейнеров. Этот запрет больше не действует. Вы можете переписать ваш `FlatMap` на использование, например, `std::vector` вместо обычных указателей и массивов - это упростит код и, возможно, реализацию некоторых допов.
