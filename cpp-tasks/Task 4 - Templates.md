...
8. Сделайте так, чтобы `FlatMap` корректно работал для случаев: `Key = const T`, `Value = const V`, `Key = T&`, `Value = V&` (где `T` и `V` - произвольные типы) и их комбинаций (например, `Key = const T&, Value = V&`). Некоторые методы для некоторых комбинаций могут не компилироваться, это нормально (например, `operator[]` для случаев `Value = const V`). Но в целом работоспособность класса должна сохраняться.
