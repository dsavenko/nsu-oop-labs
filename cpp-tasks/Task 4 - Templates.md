...
Звездочкой (*) помечены дополнительные условия, которые *могут быть* сложнее остальных. Рекомендует брать их в последнюю очередь.
...
2. Реализуйте поддержку списков инициализации (initializer lists), а именно, чтобы работал код наподобие `FlatMap<std::string, int> m = { {"a", 1}, {"b", 2} };`
...
8. (\*) Сделайте так, чтобы `FlatMap` корректно работал для случаев: `Key = const T`, `Value = const V`, `Key = T&`, `Value = V&`, `Key = T&&`, `Value = V&&` (где `T` и `V` - произвольные типы) и их комбинаций (например, `Key = const T&, Value = V&&`). Некоторые методы для некоторых комбинаций могут не компилироваться, это нормально (например, неконстантный `operator[]` для случаев `Value = const V`). Но в целом работоспособность класса должна сохраняться.
