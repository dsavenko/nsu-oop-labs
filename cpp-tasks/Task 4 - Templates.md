...
2. Если колонок в CSV-файле меньше, чем параметров парсера, заполните недостающие колонки дефолтными значениями соответствующих типов.
3. Добавьте в конец кортежа, который возвращает итератор, параметр типа `std::string`, куда будут записаны все остальные колонки, если их в файле реально больше, чем шаблонных параметров парсера. То есть для примера выше итератор должен возвращать тип `std::tuple<int, std::string, double, std::string>`.
...
5. Аналогично пункту 4, выделите получение дефолтного значения колонки для пункта 2 в **стратегию**. Добавьте шаблонный функтор, который будет возвращать значение нужного типа, которое нужно вставить, если в реальном файле колонок меньше, чем параметров парсера.
