...
3. Реализуйте методы `begin()` и `end()` для `List`. Придумаёте, какой тип они должны возвращать, и какие операции должны быть реализованы у этого типа, чтобы следующий код компилировался и работал (выводил содержимое списка на экран):
```C++
List list = ...
