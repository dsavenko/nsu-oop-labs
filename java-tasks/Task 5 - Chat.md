...
1. Регистрация
    ...
    <success></success>
    ```
    При первом логине пользователя сервер сохраняет его имя и криптографический хэш пароля. Сам пароль хранить нельзя! При последующих логинах, если хэш передаваемого пароля не совпадает с сохраненным, возвращается сообщение об ошибке, и логина не происходит.
...
Если клиент хочет запросить содержимое файла, он делает эьто отдельным запросом, и сервер в этом случае отвечает только этому клиенту. Это сделано для того, чтобы экономить траффик клиентов, ведь файлы могут быть большими.
...
**Важно!** Среди дополнительных возможностей обязательно должна быть хотя бы одна, расширяющая протокол общения между клиентом и сервером и/или расширяющая функциональность сервера.
...
3. Поддержка emoji.
4. Возможность для пользователя ввести информацию о себе, а другим пользователям - ее посмотреть.
5. Отдельное окно (область основного окна) на клиенте со списком всех файлов, имеющихся на сервере, и возможностью любой из них скачать.
6. Закачка произвольной части файла, а не файла целиком. Может понадобится для докачки большого файла после обрыва соединения.
7. Показывать индикатор прогресса закачки файла на клиенте.
8. Реализовать отдачу сервером файлов дополнительно по протоколу HTTP.
9. **Придумайте ваши собственные улучшения чата и реализуйте их!**
